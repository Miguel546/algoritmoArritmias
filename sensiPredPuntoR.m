function [VP,FP,FN, Sensi, Predp, VParreglo, FParreglo, FNarreglo]=sensiPredPuntoR(algoritmo, anotaciones)
anotacionesBD = anotaciones.('Sample');
anotacionesBDType = anotaciones.('Type');
VP=0;
FP=0;
...
matriz(1,1) = "";
 for k=1:length(algoritmo)
     ...
     for n=1:length(anotacionesBD)
         ...
         if(algoritmo(k)<=anotacionesBD(n)+10 && algoritmo(k)>=anotacionesBD(n)-10)
             matriz(k+1, n+1) = 1;
             VP=VP+1;
             FN=FN-1;
             arrct=1;
