function [ecg, ecgs, Rindex, Q_index, S_index, K_index, anotacion, locs, ecg_hplot, ecg_dplot, ecg_splot, ecg_mplot, qrs_i, qrs_c, NOISL_buf, SIGL_buf, THRS_buf, qrs_i_raw,qrs_amp_raw, NOISL_buf1, SIGL_buf1, THRS_buf1] = detectarPuntoR(conexionBD, registromit, numero, fs, gr, random, muestras)
...
ecg = val;
ecg = ecg(:);
ecg = ecg(random:random+muestras-1);
...
for m=1:length(qrs_i_raw)
    ...
    if(qrs_i_raw(m) < 6)
        [Rpks(m) Rindex(m)]=max(ecgs(qrs_i_raw(m):qrs_i_raw(m)+5));
        Rindex(m)=Rindex(m)+qrs_i_raw(m);
    else
        if(qrs_i_raw(m) + 5 > length(ecgs))
            [Rpks(m) Rindex(m)]=max(ecgs(qrs_i_raw(m)-5:length(ecgs)));
