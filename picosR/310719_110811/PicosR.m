...
% Last Modified by GUIDE v2.5 19-Jun-2019 14:50:33
...
%VParregloR= evalin('base',VParregloR);
%FParregloR = evalin('base',FParregloR);
%FNarregloR = evalin('base',FNarregloR);
axes(ecgPlotR);
...
set(handles.VPR,'data',[])
set(handles.VPR, 'data', VParregloR);

set(handles.FPR,'data',[])
set(handles.FPR, 'data', FParregloR);

set(handles.FNR,'data',[])
%set(handles.FNR, 'data', cell2table(FNarregloR));
for i = 1 : size(FNarregloR,1)
    for y = 1 : 5
        if(y == 5)
        else
            FNarregloRR{i,y} = string(FNarregloR(i,y));
        end        
    end
end
%T = table(FNarregloR(:,1),FNarregloR(:,2), FNarregloR(:,3), FNarregloR(:,4), FNarregloR(:,5), 'VariableNames',{'Gender','Age','State','Vote','Hola'});
%assignin('base','FNtable', T);
set(handles.FNR, 'data', cell2table(FNarregloRR));
